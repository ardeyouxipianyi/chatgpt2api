...
| 流式输出支持 | ✅  | 已实现。 |
| 图片尺寸参数 | ✅  | 已支持 `1:1`、`16:9`、`9:16`、`4:3`、`3:4` 等比例，后端会将尺寸要求注入图片提示词，前端工作台也提供比例选择。 |
| 服务端图片 URL 缓存 | ✅  | 已实现。 |
| `rt_token` 刷新 | ❌  | 待实现。 |
| 代理配置功能 | ✅  | 已支持网页端配置全局 HTTP / HTTPS / SOCKS5 / SOCKS5H 代理，并应用到出站请求。 |
| Anthropic Messages 协议支持 | ✅  | 已支持 `POST /v1/messages`，兼容 `x-api-key` 鉴权、`anthropic-version` 请求头、非流式与流式输出。 |
